    ...
    const char *rule_message)
{
    ...
    if (action_message == NULL) {
        message = apr_psprintf(msr->mp, "%s", msre_format_metadata(msr, actionset));
    }
    else {
        message = apr_psprintf(msr->mp, "\"action_message\": \"%s\",%s", action_message,
            msre_format_metadata(msr, actionset));
